...
plt.fill_between(df_previsao.index, df_previsao['limite_superior'], df_previsao['limite_inferior'], color='grey', alpha=0.2)import pandas as pd
import numpy as np
from statsmodels.tsa.holtwinters import ExponentialSmoothing
import matplotlib.pyplot as plt
...
params = fit.params
equation_text = f'Previsão: Y = {params["smoothing_level"]:.2f}'
if "smoothing_slope" in params:
    equation_text += f' + {params["smoothing_slope"]:.2f}t'
if "smoothing_seasonal" in params:
    equation_text += f' + {params["smoothing_seasonal"]:.2f}S'
...
plt.plot(df_previsao.index, df_previsao['limite_superior'], label='Limite Superior', color='red', linestyle='--')
for x, y in zip(df_previsao.index, df_previsao['limite_superior']):
    plt.text(x, y, f'{y:.2f}', fontsize=9, ha='right')

plt.plot(df_previsao.index, df_previsao['limite_inferior'], label='Limite Inferior', color='orange', linestyle='--')
for x, y in zip(df_previsao.index, df_previsao['limite_inferior']):
    plt.text(x, y, f'{y:.2f}', fontsize=9, ha='right')
