...
# Função para ajustar modelo SARIMA e prever
def ajustar_modelo_sarima(series):
    model_sarima = SARIMAX(series, order=(6,1,0), seasonal_order=(1,1,1,12))
    model_fit_sarima = model_sarima.fit(disp=False)
    forecast_sarima = model_fit_sarima.forecast(steps=20)
    return forecast_sarima, model_fit_sarima

previsoes_sarima, fit_sarima = ajustar_modelo_sarima(df['resultado'])

# Calcular intervalo de confiança para SARIMA
sigma_sarima = np.std(fit_sarima.resid, ddof=1)  # desvio padrão dos resíduos

df_previsao_sarima = pd.DataFrame({
    'previsão': previsoes_sarima,
    'limite_superior': previsoes_sarima + z * sigma_sarima,
    'limite_inferior': previsoes_sarima - z * sigma_sarima
}, index=pd.date_range(start=df.index[-1] + pd.offsets.MonthEnd(1), periods=10, freq='MS'))

# Filtra apenas os dados de 2024 para plotagem
df_2024 = df.loc['2024']
df_previsao_hw_2024 = df_previsao_hw.loc['2024']
df_previsao_sarima_2024 = df_previsao_sarima.loc['2024']
df_meta_2024 = df_meta.loc['2024']

# Ajustar um polinômio aos pontos previstos pelo SARIMA
x_vals = np.arange(len(df_previsao_sarima_2024))
y_vals = df_previsao_sarima_2024['previsão'].values
...
plt.plot(df_previsao_sarima_2024.index, df_previsao_sarima_2024['previsão'], label='Previsão SARIMA', linestyle='--', marker='o')
for x, y in zip(df_previsao_sarima_2024.index, df_previsao_sarima_2024['previsão']):
    plt.text(x, y, f'{y:.2f}', fontsize=9, ha='right')
...
plt.fill_between(df_previsao_sarima_2024.index, df_previsao_sarima_2024['limite_superior'], df_previsao_sarima_2024['limite_inferior'], color='grey', alpha=0.2)

# Plotar a linha de ajuste polinomial
#plt.plot(df_previsao_sarima_2024.index, y_fit, label=f'Ajuste Polinomial: {equation}', linestyle='-', color='blue')

plt.title(f'Previsões com SARIMA e Limites de Confiança\n({int(confidence_level*100)}% Intervalo de Confiança)')
plt.xlabel('')
plt.ylabel('')
plt.legend()
plt.grid(True)
...
df_complete_sarima = pd.concat([df, df_previsao_sarima], axis=0)
df_complete_sarima['meta'] = df_meta['meta']
df_complete_sarima.to_csv('resultados_previsoes_sarima.csv', decimal=',')
